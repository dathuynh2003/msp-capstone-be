...
namespace MSP.Tests.Services.AccountServicesTest
{
    public class LoginTest
    {
        ...
        public async Task LoginAsync_UpdatesRefreshTokenInDatabase()
        {
            ...
            _mockAuthTokenProcessor
                .Setup(x => x.GenerateJwtToken(user, roles))
                .Returns(("jwt_token", expiresAtUtc));
            ...
            Assert.NotNull(user.RefreshTokenExpiresAtUtc);

            _mockUserManager.Verify(x => x.UpdateAsync(user), Times.Once);
        }

        [Fact]
        public async Task LoginAsync_WritesCookiesForTokens()
        {
            // Arrange
            var loginRequest = new LoginRequest
            {
            ...
            // Assert
            _mockAuthTokenProcessor.Verify(
                x => x.WriteAuthTokenAsHttpOnlyCookie("ACCESS_TOKEN", jwtToken, expiresAtUtc),
                Times.Once
            );

            _mockAuthTokenProcessor.Verify(
                x => x.WriteAuthTokenAsHttpOnlyCookie("REFRESH_TOKEN", refreshToken, It.IsAny<DateTime>()),
                Times.Once
            );
        }
